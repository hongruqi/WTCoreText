...
typedef enum WTDisplayViewState : NSInteger {
    WTDisplayViewStateNormal,       // 普通状态
    WTDisplayViewStateTouching,     // 正在按下，需要弹出放大镜
    WTDisplayViewStateSelecting     // 选中了一些文本，需要弹出复制菜单
}WTDisplayViewState;
...
- (void)setData:(WTTextData *)data {
    _data = data;
    self.state = WTDisplayViewStateNormal;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.state == WTDisplayViewStateTouching || self.state == WTDisplayViewStateSelecting) {
    ...
    for (WTImageData *imageData in self.data.imageArray) {
...
- (void)setState:(WTDisplayViewState)state {
    if (_state == state) {
        return;
    }
    _state = state;
    if (_state == WTDisplayViewStateNormal) {
        _selectionStartPosition = -1;
        _selectionEndPosition = -1;
        [self removeSelectionAnchor];

    } else if (_state == WTDisplayViewStateTouching) {
        if (_leftSelectionAnchor == nil && _rightSelectionAnchor == nil) {
            [self setupAnchors];
        }
    } else if (_state == WTDisplayViewStateSelecting) {
        if (_leftSelectionAnchor == nil && _rightSelectionAnchor == nil) {
            [self setupAnchors];
        }
    ...
    if (_state == WTDisplayViewStateNormal) {
        for (WTImageData *imageData in self.data.imageArray) {
            ...
            if (CGRectContainsPoint(rect, point)) {
                NSLog(@"hint image");
                // 在这里处理点击后的逻辑
                if ([self.delgate respondsToSelector:@selector(wt_displayView:imageDidClick:)]) {
                    [self.delgate wt_displayView:self imageDidClick:imageData];
                }
        ...
        WTLinkData *linkData = [UIView touchLinkInView:self atPoint:point data:self.data];
        if (linkData) {
            NSLog(@"hint link!");
            if ([self.delgate respondsToSelector:@selector(wt_displayView:linkDidClick:)]) {
                [self.delgate wt_displayView:self linkDidClick:linkData];
            }
            return;
        }
    } else {
...
- (void)userPanGuestureDetected:(UIGestureRecognizer *)recognizer {
    if (self.state == WTDisplayViewStateNormal) {
        return;
    }
    CGPoint point = [recognizer locationInView:self];
    if (recognizer.state == UIGestureRecognizerStateBegan) {
